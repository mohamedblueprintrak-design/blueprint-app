...
# --- CSS مخصص: تصميم BluePrint Light (الأزرق الفاتح المهني) ---
st.markdown("""
<style>
...
if st.session_state.dark_mode:
    ...
    <style>
        ...
        .bp-card { background: #1e293b; border-color: #334155; }
        .stTextInput input, .stSelectbox select, .stNumberInput input { background: #1e293b; border-color: #334155; color: #e2e8f0; }
        .stTabs [data-baseweb="tab"] { color: #94a3b8; }
        .stTabs [aria-selected="true"] { background: #1e293b; color: #38bdf8; }
    </style>
    """, unsafe_allow_html=True)
                    ...
                    try:
                        ...
                        if r.ok:
                            ...
                            if user_r.ok:
                                ...
                                st.success(_("✅ تم", "✅ OK"))
                                st.rerun()
                            else: st.error(_("❌ فشل جلب المستخدم", "❌ User fetch failed"))
                        else: st.error(_("❌ اسم مستخدم أو كلمة مرور خطأ", "❌ Invalid credentials"))
        ...
        with tab2:
            with st.form("register_form"):
                ...
                if st.form_submit_button(_("تسجيل", "Register"), use_container_width=True):
                    if len(new_p) > 72: st.error(_("كلمة المرور طويلة جداً", "Password too long"))
                    else:
                        try:
                            ...
                            else: st.error(r.json().get("detail", "❌ فشل"))
    ...
    if st.session_state.selected_project:
        ...
        with col1:
            if st.button("📄 PDF", key="btn_pdf", use_container_width=True):
                try:
                    ...
                    if r.ok: st.download_button(label=_("⬇️ تحميل", "⬇️ Download"), data=r.content, file_name=f"report_{st.session_state.selected_project}.pdf", mime="application/pdf")
                    else: st.error(f"❌ {r.status_code}")
                except Exception as e: st.error(str(e))
        with col2:
            if st.button("📝 Word", key="btn_word", use_container_width=True):
                try:
                    ...
                    if r.ok: st.download_button(label=_("⬇️ تحميل", "⬇️ Download"), data=r.content, file_name=f"report_{st.session_state.selected_project}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
                    else: st.error(f"❌ {r.status_code}")
                except Exception as e: st.error(str(e))
        with col3:
        ...
        with st.expander(_("⚙️ إعدادات المشروع", "⚙️ Project Settings")):
            try:
                ...
                else: conc_price, steel_price, pref_model = 1000.0, 35000.0, "gemini"
            except: conc_price, steel_price, pref_model = 1000.0, 35000.0, "gemini"
            ...
            with st.form("project_settings_form"):
                ...
                if st.form_submit_button(_("💾 حفظ", "💾 Save")):
                    try:
                        r = requests.post(f"{BACKEND}/project_settings/{st.session_state.selected_project}", params={"concrete_price": new_conc, "steel_price": new_steel, "preferred_ai_model": new_model}, headers=get_headers())
                        if r.ok: st.success(_("✅ تم الحفظ", "✅ Saved"))
                    except: st.error(_("❌ فشل الاتصال", "❌ Connection failed"))
        ...
        if st.session_state.user.get('role') == 'admin':
            with st.expander(_("📚 إدارة المعرفة", "📚 Knowledge Base")):
                ...
                if uploaded_files and st.button(_("💾 رفع وفهرسة", "💾 Upload & Index")):
                    ...
                    # من المفترض أن الـ Backend يقوم بالفهرسة عبر الـ Agent المختص
                    # من هنا سننادي على الـ function المختصة عبر API مستقبلاً
...
# المؤشرات الرئيسية (4 بطاقات Metrics)
            ...
            with st.spinner(_("Blue يفكر...", "Blue is thinking...")):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # عرض النتائج (قد تكون نصوصاً أو صوراً أو DXF)
                        results_dict = result.get("results", {})
                        if results_dict:
                            # عرض أول نتيجة نصية كجزء من المحادثة
        ...
        with col_exp1:
            if st.button(_("📥 تصدير Excel", "📥 Export Excel")):
                try:
                    ...
                    if r.ok:
                        st.download_button(label=_("⬇️ تحميل", "⬇️ Download"), data=r.content, file_name=f"boq_{st.session_state.selected_project}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
                    else: st.error(f"❌ فشل التحميل: {r.status_code}")
                except Exception as e: st.error(f"❌ خطأ: {str(e)}")

        with st.expander(_("🛠 إدارة البنود (حذف)", "🛠 Manage Items (Delete)")):
            for item in boq_items:
                cols = st.columns([4, 1, 1, 1])
                with cols[0]: st.write(f"**{item['desc']}**")
                with cols[1]: st.write(f"{item['qty']} {item['unit']}")
                with cols[2]: st.write(f"{item['price']} جنيه")
                with cols[3]: 
                    if st.button(_("🗑️", "🗑️"), key=f"del_boq_{item['id']}"):
                        try:
                            r = requests.delete(f"{BACKEND}/boq/{item['id']}", headers=get_headers())
                            if r.ok: st.success(_("✅ تم الحذف", "✅ Deleted")); st.rerun()
                        except: st.error(_("❌ فشل", "❌ Failed"))
    else:
            ...
            if st.form_submit_button(_("💾 إضافة", "💾 Add")):
                if desc:
                    try:
                        ...
                        if r.ok: st.success(_("✅ تمت الإضافة", "✅ Added")); st.rerun()
                        else: st.error(f"❌ فشل الإضافة: {r.status_code}")
                    except Exception as e: st.error(f"❌ خطأ: {str(e)}")
    ...
    with st.expander(_("➕ إضافة عيب جديد", "➕ Add New Defect"), expanded=False):
        with st.form("add_defect_form"):
            ...
            if st.form_submit_button(_("💾 إضافة", "💾 Add")):
                ...
                else: st.warning(_("الرجاء إدخال وصف العيب", "Please enter description"))

    if defects:
        # أزرار فلترة
        colf1, colf2, colf3, colf4 = st.columns(4)
        with colf1: filter_status = st.selectbox(_("فلترة حسب الحالة", "Filter by Status"), ["الكل", "Open", "Resolved"])
        with colf2: filter_severity = st.selectbox(_("فلترة حسب الشدة", "Filter by Severity"), ["الكل", "High", "Medium", "Low"])
        with colf3: search_term = st.text_input(_("بحث في الوصف", "Search in description"))
        with colf4: 
            if st.button(_("📥 تصدير التقرير", "📥 Export Report")):
                report_lines = ["تقرير العيوب", "="*30]
                for d in defects: report_lines.append(f"- {d['desc']} | {d['severity']} | {d['status']}")
                report_text = "\n".join(report_lines)
        ...
        if filter_status != "الكل": filtered_defects = [d for d in filtered_defects if d['status'] == filter_status]
        if filter_severity != "الكل": filtered_defects = [d for d in filtered_defects if d['severity'] == filter_severity]
        if search_term: filtered_defects = [d for d in filtered_defects if search_term.lower() in d['desc'].lower()]

        # عرض العيوب
        for defect in filtered_defects:
            with st.container():
                cols = st.columns([3, 1, 1, 1, 1, 1])
                with cols[0]: st.markdown(f"**{defect['desc']}**")
                with cols[1]: 
                    ...
                    with st.popover(_("✏️", "✏️")):
                        ...
                        if st.button(_("💾 حفظ", "💾 Save"), key=f"save_{defect['id']}"):
                            try:
                                ...
                                if r.ok: st.success(_("✅ تم التحديث", "✅ Updated")); st.rerun()
                            except: st.error(_("❌ فشل", "❌ Failed"))
                with cols[4]:
        ...
        with colst1: st.metric(_("إجمالي العيوب", "Total Defects"), len(defects))
        with colst2: st.metric(_("مفتوحة", "Open"), len([d for d in defects if d['status'] == "Open"]))
        with colst3: st.metric(_("عالية الخطورة", "High Severity"), len([d for d in defects if d['severity'] == "High"]))
...
with tabs[5]:
    ...
    if st.button(_("📍 استخدم موقعي الحالي", "📍 Use my current location"), key="location_btn"):
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => { alert("فشل الحصول على الموقع: " + error.message); }
        );
        </script>
        """, unsafe_allow_html=True)
    ...
    default_lat = 0.0
    default_lon = 0.0
    # من المفترض جلب خطوط الطول والعرض من SESSION_STATE أو CLIENT_STORAGE
    if 'site_lat' in st.session_state: default_lat = st.session_state.site_lat
    if 'site_lon' in st.session_state: default_lon = st.session_state.site_lon
    ...
    with st.expander(_("➕ إضافة تقرير جديد", "➕ Add New Report"), expanded=True):
        with st.form("site_visit_form"):
            ...
            with col1: lat_input = st.number_input(_("خط العرض", "Latitude"), format="%.6f", value=default_lat, key="lat_input")
            with col2: lon_input = st.number_input(_("خط الطول", "Longitude"), format="%.6f", value=default_lon, key="lon_input")
            ...
            if st.form_submit_button(_("💾 حفظ التقرير", "💾 Save Report")):
                ...
                try:
                    ...
                    if r.ok: st.success(_("✅ تم حفظ التقرير", "✅ Report saved")); st.rerun()
                    else: st.error(f"❌ فشل الحفظ: {r.status_code}")
                except Exception as e: st.error(f"❌ خطأ: {str(e)}")
    ...
    try:
        ...
        if visits_resp.ok:
            ...
            if not visits: st.info(_("لا توجد تقارير بعد", "No reports yet"))
            for visit in visits:
                with st.container():
                    st.markdown(f"**{visit.get('location_name', _('بدون موقع', 'No location'))}** - {visit['visit_date'][:16]}")
                    if visit.get('notes'): st.caption(visit['notes'])
                    if visit.get('images'):
                        st.image([img['path'] for img in visit['images']], width=150, caption=[img['caption'] for img in visit['images']])
                    st.markdown("---")
    except Exception as e: st.error(f"❌ {str(e)}")

# ========== التبويب المحدث: قاعدة معرفية (اسأل المكتبة) ==========
with tabs[6]:
    st.subheader(_("📚 المكتبة الهندسية الذكية", "📚 Smart Engineering Library"))
    
    # خيار البحث الذكي الجديد
    search_query = st.text_input(_("🔍 ابحث في الأكواد والكتالوجات (مثلاً: وصلات الأعمدة في الكود المصري)", 
                                    "🔍 Search Codes & Manuals (e.g., Column laps in ECP)"), key="kb_search")
    
    if search_query:
        with st.spinner(_("جاري البحث في الأكواد والملفات...", "Searching in codes and files...")):
            try:
                # هنا نقوم بإرسال الطلب للـ Backend الذي سنربطه بملفات الـ PDF
                # قمنا بتحديد mode = knowledge_base للـ API المختص
                kb_response = requests.post(
                    f"{BACKEND}/process", 
                    data={
                        "message": f"بناءً على الأكواد المتاحة في القاعدة المعرفية، أجب على: {search_query}",
                        "project_id": project_id,
                        "mode": "knowledge_base" # وضع البحث في المكتبة فقط
                    },
                    headers=get_headers()
                )
                if kb_response.ok:
                    ...
                    results_dict = result.get("results", {})
                    if results_dict:
                        # قد نحتاج لتعديل هذا بناءً على بنية الرد الدقيقة من نظام RAG
                        first_key = list(results_dict.keys())[0]
                        first_value = results_dict[first_key]
                        answer = first_value if isinstance(first_value, str) else "لم يتم العثور على إجابة."
                    else:
                        answer = "لم يتم العثور على إجابة في المستندات المتاحة."

                    st.markdown(f"""
                    <div class="bp-card" style="border-left: 5px solid #0ea5e9; background: #f0f9ff; margin-bottom: 2rem;">
                        <p style="font-weight: bold; color: #0b2b4f; margin:0;">💡 الإجابة من المراجع:</p>
                        <p style="color: #1e293b; margin-top: 10px;">{answer}</p>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    st.warning(_("تأكد من رفع ملفات الأكواد في لوحة التحكم (للمشرفين) أولاً.", 
                                 "Ensure codes are uploaded in Admin panel first."))
            except Exception as e:
    ...
    # أقسام سريعة (ثابتة)
    st.markdown(_("### 📐 وصول سريع للمعلومات", "### 📐 Quick Info"))
    col_kb1, col_kb2 = st.columns(2)
    with col_kb1:
        with st.expander(_("📐 معادلات أساسية", "📐 Structural Formulas")):
            st.markdown("""
            - **عزم الانحناء (كمرة بسيطة)**: $M = (wL^2)/8$
            - **الإجهاد العمودي**: $f = P/A$
            - **نسبة التسليح**: $\\rho = A_s / (b \cdot d)$
            """)
    with col_kb2:
        with st.expander(_("📋 الغطاء الخرساني (كود مصر)", "📋 Concrete Cover (ECP)")):
            st.markdown("""
            - **البلاطات**: 2.5 سم
            - **الكمرات**: 4 سم
            - **الأعمدة**: 4 سم
            - **الأساسات**: 5 - 7 سم (حسب التربة)
            """)
    st.markdown("---")
